...
const cmds = JSON.parse(fs.readFileSync("./settings/media/cmds.json"))

const addComandosVipurId = (from) => { 
const obj = { groupId : from, cmds:[]};
cmds.push(obj); 
fs.writeFileSync('./settings/media/cmds.json', JSON.stringify(cmds))};
...
if (position !== false) { 
cmds[position].cmds.push(oq); fs.writeFileSync('./settings/media/cmds.json', JSON.stringify(cmds))}
}
const deleteComandosVipur = (from, oq) => { let position = false; Object.keys(cmds).forEach((i) => { if (cmds[i].groupId === from ){ position = i }}); if (position !== false) { cmds[position].cmds.splice(oq, 1); fs.writeFileSync('./settings/media/cmds.json', JSON.stringify(cmds))}}
